...
document.addEventListener('DOMContentLoaded', function () {
  // ===== OPERATOR STATUS =====
function syncOperatorStatus(status) {
  if (!window.firebaseDB) return;
  if (!state.operator) return;

  window.firebaseUpdate(
    window.firebaseRef(window.firebaseDB, "operators/" + state.operator),
    {
      status: status,
...
function syncShiftStart() {
  ...
  window.firebaseSet(
    window.firebaseRef(window.firebaseDB, "shifts/" + id),
    {
...
function syncShiftUpdate() {
  if (!fb()) return;
  const id = getShiftId();
  if (!id) return;

  const calls = state.projects.reduce((s,p)=>s+p.calls,0);
...
// === END FIREBASE ===
  ...
  state.mainTimer = setInterval(() => {
    ...
    // ⬇️ ВОТ ОНО, БРО
    let status = "finished";
    if (state.shiftActive) status = "active";
    if (state.currentCall) status = "call";
    if (state.shiftPaused) status = "pause";

    syncOperatorStatus(status);
...
// ======================
 ...
 // ===== PERSONAL NOTES (PER SHIFT) =====
function getCurrentShiftNotesKey() {
  // 1. если есть сохранённый ключ — используем его
  const savedKey = localStorage.getItem("pulse_current_shift_notes");
  if (savedKey) return savedKey;

  // 2. если смена активна — создаём новый
  if (state.operator && state.shiftStartTime) {
    const key = `pulse_notes_${state.operator}_${state.shiftStartTime}`;
    localStorage.setItem("pulse_current_shift_notes", key);
    return key;
  }
...
function initPersonalNotesPerShift() {
  const textarea = document.getElementById("personal-notes");
  if (!textarea) return;

  const key = getCurrentShiftNotesKey();
  if (!key) return;
...
window.firebaseOnValue(
  window.firebaseRef(window.firebaseDB, "shifts"),
  snapshot => {
...
function renderTop3() {
  ...
  const list = allShifts
    .filter(s => typeof s.calls === "number")
  ...
  if (!list.length) {
    box.innerHTML = "<div style='opacity:.6;text-align:center'>Нет данных</div>";
    return;
  }
